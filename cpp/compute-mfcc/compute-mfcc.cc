...
int main(int argc, char* argv[]) {
    ...
    // Assign variables
    int numCepstra = (numCepstraC ? atoi(numCepstraC) : 13);
    int numFilters = (numFiltersC ? atoi(numFiltersC) : 40);
    int samplingRate = (samplingRateC ? atoi(samplingRateC) : 48000);
    int winLength = (winLengthC ? atoi(winLengthC) : 2048);
    int frameShift = (frameShiftC ? atoi(frameShiftC) : 512);
